...
When a configured OpenAI-compatible provider points at `localhost`, `127.0.0.1`, or `::1`, Loopi places a run lock under `shared/.locks/`.
...
Transport compatibility does not imply model trust. If you point Loopi at a local or self-hosted endpoint, you are responsible for validating the model source, prompt handling, and any surrounding infrastructure.
